<template>
    ...
    <el-table :data="tableData" style="width: 100%">
        <el-table-column prop="levelId" label="ID" width="120" />
        <el-table-column prop="levelName" label="头衔名称" width="120" />
        <el-table-column prop="description" label="头街描述" width="120" />
        <el-table-column prop="amount" label="等级必要金额" width="120" />
  ...
  */
  ...
  const initTableData = async () => {
    try {
        const res = await getAllUserlevel();      // 改这里，获得所用数据
        console.log(res);   ///////
        if (res.success) {
            tableData.value = res.data.userLevels;   // 改这里，res.data.XXX (看控制台的输出)
